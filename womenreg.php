    ...
    if (isset($_POST['submit'])) {
        ...
        if (array_key_exists($scheme_name, $schemes)) {
            ...
            if (mysqli_num_rows($check_query) > 0) {
                ...
                echo "<script>setTimeout(function() { window.location.href = 'womenreg.html'; }, 1500);</script>";
            } else {
                ...
                $query = mysqli_prepare($con, "INSERT INTO scheme_reg (Scheme_name, Name, Age, age_img, District, Taluk, Phone_No, Marital_status, Image, women_id) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

                mysqli_stmt_bind_param($query, "ssssssssss", $scheme_name, $name, $age, $age_proof_data, $district, $taluk, $phone, $marital_status, $unique_name, $women_id);

                if (mysqli_stmt_execute($query)) {
                    echo "Registration successful";
                    echo "<script>setTimeout(function() { window.location.href = 'women.html'; }, 1500);</script>";
                } else {
